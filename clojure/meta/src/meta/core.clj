...
; TODO: test
(defn bcode-from-seq [bseq]
  "Lazily lists bcode fragments that can be generated by one step of
  bcode-with-fragment from bcode provided in the input sequence."
  (mapcat (fn [prefix]
            (bcode-with-fragment (last prefix) (butlast prefix))
            (prefixes bseq)
            )))
...
(defn all-bcode-sorted-by [sortfn]
  "Lazily lists all valid Boson expressions, sorted by sortfn assuming
  that sortfn is always smaller on subtrees."
